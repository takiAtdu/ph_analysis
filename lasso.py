...
filename = "lasso_" + phase + str(dimension) + ".sav"
...
df_result = pd.DataFrame(data=[[lss.score(X_train, y_train)], [lss.score(X_test, y_test)], [X.shape[1]], 
                            [np.sum(lss.coef_ != 0)], [mean_squared_error(y_test,y_pred)], 
                            [avg_1600c3h], [avg_1600c24h], [avg_1700c3h], 
                            [avg_1800c3h], [avg_1800c24h], [avg_ascast]], 
                    index = ["train score", "test score", "元の特徴量の数", 
                            "Lassoの特徴量", "最小2乗誤差", 
                            "1600c3hの平均値", "1600c24hの平均値", "1700c3hの平均値", 
                            "1800c3hの平均値", "1800c24hの平均値", "ascastの平均値"], 
                    columns = ["値"])
df_result.to_csv(save_dir + "lasso_" + phase + str(dimension) + ".csv")
print(df_result)
...
plt.savefig(save_dir + "lasso_" + str(phase) + str(dimension) + ".png")
# plt.show()